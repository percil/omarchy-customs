...
Panel {
  ...
  // "" when idle, otherwise which action is in flight — drives the button
  // labels below so a slow systemctl call reads as busy, not as ignored.
  property string ollamaPendingAction: ""

  Process {
    id: ollamaControlProcess
    running: false
    onExited: {
      root.ollamaPendingAction = ""
      ollamaSettleTimer.restart()
    }

    stderr: StdioCollector {
      waitForEnd: true
      onStreamFinished: if (text.trim() !== "") console.warn("agents/ollama", text.trim())
    }
  }

  // systemctl start/stop returns as soon as it has issued the request; give
  // the server a moment to actually bind or release the port before the
  // status record is regenerated, or the refresh can read the old state.
  Timer {
    id: ollamaSettleTimer
    interval: 600
    repeat: false
    onTriggered: usage.runOllamaUpdate()
  }

  // Ollama also gets started by hand from a terminal (`ollama serve`), not
  // just through the systemd unit — so Start and Stop are two independent
  // buttons rather than one label that toggles off the last-known state.
  // Start always goes through systemd (idempotent — a no-op if the unit or
  // a manual instance already has the port); Stop kills either kind of
  // process, so it works whether or not this session ever started it.
  function startOllama() {
    if (ollamaControlProcess.running) return
    root.ollamaPendingAction = "start"
    ollamaControlProcess.command = ["bash", "-c", "systemctl --user start ollama.service"]
    ollamaControlProcess.running = true
  }

  function stopOllama() {
    if (ollamaControlProcess.running) return
    root.ollamaPendingAction = "stop"
    ollamaControlProcess.command = ["bash", "-c",
      "systemctl --user stop ollama.service; pkill -u \"$USER\" -f '(^|/)ollama serve$' 2>/dev/null; true"]
    ollamaControlProcess.running = true
  }
    ...
    PanelKeyCatcher {
      ...
      Flickable {
        ...
        Column {
          ...
          // ---------- Ollama: installed models + start/stop ----------
          // Ollama has no usage/rate-limit API to show, so it gets its own
          // small section at the bottom instead of the limits/tokens ones
          // above: the models it has pulled, then separate Start/Stop
          // buttons (not one toggle — Ollama is just as often started by
          // hand from a terminal, so the panel shouldn't assume it owns the
          // only path to "running"). Kept below the provider switch so the
          // Claude Code / Codex / Ollama tabs always stay put at the top.
          PanelSeparator {
            visible: ollamaModelsSection.visible || ollamaControls.visible
            foreground: root.foreground
          }

          Column {
            id: ollamaModelsSection
            visible: !!root.provider && root.provider.providerId === "ollama" && root.provider.models && root.provider.models.length > 0
            width: parent.width
            ...
            Repeater {
              model: root.provider ? (root.provider.models || []) : []

              Text {
                required property var modelData
                textFormat: Text.PlainText
                width: ollamaModelsSection.width
          ...
          Row {
            id: ollamaControls
            visible: !!root.provider && root.provider.providerId === "ollama"
            width: parent.width
            ...
            Button {
              width: ollamaControls.cellWidth
              text: root.ollamaPendingAction === "start" ? "…" : "Start Ollama"
              bordered: true
              foreground: root.foreground
              fontFamily: root.fontFamily
              fontSize: Style.font.bodySmall
              verticalPadding: Style.spacing.controlPaddingY
              onClicked: root.startOllama()
            }

            Button {
              width: ollamaControls.cellWidth
              text: root.ollamaPendingAction === "stop" ? "…" : "Stop Ollama"
              bordered: true
              foreground: root.provider && root.provider.running ? root.urgent : root.foreground
              fontFamily: root.fontFamily
              fontSize: Style.font.bodySmall
              verticalPadding: Style.spacing.controlPaddingY
              onClicked: root.stopOllama()
            }
            ...
            Button {
              ...
              fontSize: Style.font.bodySmall
              verticalPadding: Style.spacing.controlPaddingY
              onClicked: usage.runOllamaUpdate()
            }
          }
