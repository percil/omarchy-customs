...
Item {
  ...
  function runUpdate(kind, agentIds) {
    if (!agentIds || agentIds.length === 0 || agentIds.indexOf("ollama") >= 0) runOllamaUpdate()
  ...
  // ------------------------------------------------------------ ollama
  //
  // Ollama has no packaged collector under $OMARCHY_PATH/bin (that directory
  // is never touched by end-user customization), so its record is produced
  // by a user-owned collector and written straight into the usage directory
  // here, riding along on the same refresh cadence as the packaged agents.

  Process {
    id: ollamaUpdateProcess
    running: false
  ...
  function runOllamaUpdate() {
    if (!providerEnabled("ollama")) return
    if (ollamaUpdateProcess.running) return
    var dest = root.usageDir + "/ollama.json"
    ollamaUpdateProcess.command = ["bash", "-c",
      "mkdir -p \"$(dirname \"$1\")\" && tmp=$(mktemp \"$1.XXXXXX\") && omarchy-agent-usage-ollama >\"$tmp\" && mv \"$tmp\" \"$1\"",
      "_", dest]
    ollamaUpdateProcess.running = true
  }
  ...
  function providerHasData(p) {
    return numberValue(p.totalPrompts) > 0 || numberValue(p.totalSessions) > 0
      ...
      || !!p.balance
      // Ollama (and any other agent with no prompt/token telemetry) still
      // earns a tab once it exists locally: a running server or an
      // installed model is itself the signal. forceVisible lets a record
      // opt in outright — Ollama's tab stays put even while stopped, so the
      // start/stop control is always reachable.
      || p.running === true || numberValue(p.modelCount) > 0 || p.forceVisible === true
  }
  ...
  function displayProvider(record) {
    ...
    return {
      ...
      // Ollama-only fields: no equivalent in the shared record shape, so
      // they never travel through sync and simply read empty there.
      running: record.running === true,
      serviceManaged: record.serviceManaged === true,
      models: Array.isArray(record.models) ? record.models : [],
      modelCount: numberValue(record.modelCount),
      forceVisible: record.forceVisible === true
    }
  }
